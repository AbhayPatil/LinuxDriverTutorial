...
MODULE_DESCRIPTION(
    "A simple gpio driver fo setting a LED and reading a button");
...
static ssize_t driver_read(struct file *File, char *user_buffer, size_t count,
  ...
  to_copy = min((int)count, (int)sizeof(tmp));
...
/**
 * @brief Wrote data to the buffer
 */
...
static int __init ModuleInit(void) {
  ...
  /* Set GPIO 4 input Direction*/
  if (gpio_direction_input(4)) {
    printk("Cannot set GPIO 4 to input!\n");
    goto Gpio17Error;
  }
